...
import { ACTIONS, assert, timeout, MODES, URL, HTTP_STATUS_CODES, timestamp, HTTP_METHODS, buildUrl } from '@pollyjs/utils';
...
class EventEmitter {
  ...
  async emitParallel(eventName, ...args) {
    ...
    if (this.hasListeners(eventName)) {
      await promise$1.all(this.listeners(eventName).map(listener => listener(...args)));
      return true;
    }
...
class PollyRequest extends HTTPBase {
  ...
  async respond(response) {
    ...
    this.response.setHeaders(headers$$1); // Set the body without modifying any headers (instead of using .send())
  ...
  async _identify() {
    ...
    keys$2(NormalizeRequest).forEach(key => {
...
async function beforeEach(context, recordingName, defaults) {
